...
const canReadAnyUser = rule()((__: any, ___: any, { user }): boolean => {
	Logger.info('Inside canReadAnyUser Middleware')
	return checkPermission(user, 1001)
})
...
export default shield({
	Query: {
		ping: or(and(canReadOwnUser, isReadingOwnUser), canReadAnyUser, isAuthenticated),
